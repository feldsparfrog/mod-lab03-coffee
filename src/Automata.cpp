...
void Automata::Check() {
    if (State == CHECK) {
        if (Cash < Prices[ChosenPosition]) {
            ...
            std::cout << "Подтвердите выбор " << Menu[ChosenPosition] <<
            std::endl;
        }
    }
}
...
void Automata::Cook(bool printState) {
    if (State == CHECK) {
        State = COOK;
        std::cout << "Ваш " << Menu[ChosenPosition] << " готовится..." <<
            std::endl;
        if (printState) GetState();
        Cash -= Prices[ChosenPosition];
        std::cout << "..." << std::endl << "Ваш " << Menu[ChosenPosition] <<
            " готов!" << std::endl;
        Finish();
    }
}
void Automata::Finish() {
    if (State == COOK) {
        State = WAIT;
        std::cout << "Сдача: " << Cash << std::endl << "Хорошего дня!" <<
            std::endl;
        Cash = 0;
        ChosenPosition = -1;
    }
